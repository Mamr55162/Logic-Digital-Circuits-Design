...
```
A digital probe can be used in Multisim in the Indicator group under the Probe family, named PROBE_DIG, this is the suitable probe for digital circuits, it connects directly to the output of a gate or circuit with no ground connections, it lights when the wire is HIGH and turns off when the wire is LOW.
...
```
A decimal display in multisim can be found in Indicators group, under the HEX_DISPLAY family, named DCD_HEX, it can be found with many colors variations, it has four pins, each pin connects to a certain binary digit to output the number in decimal, this indicator is most applicable in decoder, MUX circuits and ICs that has too many output pins.
